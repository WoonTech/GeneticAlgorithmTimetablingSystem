...
namespace GeneticAlgorithmTimetablingSystem
{
    ...
    class LecturerInfoCompiler
    {
        ...
        /// </summary>
        public LecturerInfoCompiler()
        {
            // clear bool[,] compilerData array by false
            compiledData = new bool[9, 6];
            anyToken.Clear();
        }
            ...
            //foreach (char token in spell)
            {
                ...
                switch (token)
                {
                    ...
                    case 'F': // FREE || FRI token
                        ...
                        break;
                    case 'T': // TUE || THR token
                        {
                            buffer += token.ToString(); // buffer = "T"
                            token = spell[++index];
                            buffer += token.ToString(); // buffer = "TH"
                            token = spell[++index];
                            buffer += token.ToString(); // buffer = "TUE" or "THUR"
                            if (buffer == "TUE" || buffer == "THUR")
                            {
                                addToken(buffer, "Day");
                                buffer = string.Empty;
                            }
